...
/* ══════════════════════════════════════════════════════════════
   KALE GÖLGESİ — TABAN ELİPSİ
   ------------------------------------------------------------
   Denenen ve BIRAKILAN yollar:

   1) drop-shadow → gölge resmin SİLÜETİNDEN üretiliyor, kalenin
      dibindeki çıkıntılar da silüete dahil → yerdeki iz tırtıklı.
   2) clip-path ile sabit dörtgen → çatının yuvarlak köşeleri
      gölgede sivriliyor, her kale seviyesi elle ayar istiyor.
   3) Görselin siyaha boyanmış KOPYASI (img.bdGolge), skewX+scaleY
      ile yatırılıyordu. Doğru çalışıyordu ama YANLIŞ İŞ: ışık
      haritaya yukarıdan vuruyor, yana uzanan silüete gerek yok.
...
/* Eski silüet kopyası artık üretilmiyor; sayfa yenilenmeden önce
   doğmuş olan kalıntı varsa görünmesin. */
...
/* ══════════════════════════════════════════════════════════════
   ...
   İKİ SIRA SEKME, tek ekranda kaydırma yok:
     üst sıra  → hangi SEVİYE sürülüyor (1-5)
     alt sıra  → hangi BÖLÜM görünüyor (Çerçeve / Görsel / Düğüm)
   Düğüm bölümü kaynak/canavar etiketidir (harita.js CFG.etiket),
   ORTAKTIR — düğümün seviyesi yok, seviye sekmesi onu etkilemez.

   SINIR YOK: her sayının yanındaki kutuya elle de yazılabilir.
   Sürgünün aralığı sadece parmakla sürerken rahat etsin diye
   geniş tutuldu (±280); kutuya ondan büyüğünü de yazabilirsin.

   Kale piksel yazar (DOM zaten düğümle birlikte ölçekleniyor);
   düğüm çarpan yazar (canvas her karede zoom'a göre yeniden
   hesaplıyor, piksel yazılsa uzaklaşınca şerit düğümden kopardı).
   Düğüm sayıları bu yüzden YÜZDE cinsinden gösteriliyor.
...
/* DÜĞÜM — çarpanların 100 katı. Hepsi r (düğüm yarıçapı)
   üzerinden; yazı ve görsel BİRBİRİNDEN BAĞIMSIZ. */
var DUGUM_VARSAYILAN = {
  dPunto:   54, dYaziX: -2, dYaziY: 133,
  dGEn:    267, dGBoy: 134, dGX:  -69, dGY: -26
};
...
var sv = 1;            /* seçili seviye */
var bolum = "cerceve"; /* cerceve | gorsel | dugum */
...
/* SEKME SIRALARI — üst sıra seviye, alt sıra bölüm. */
"#bdET .sekme{display:flex;gap:4px;}" +
"#bdET .sekme.sv{margin-bottom:5px;}" +
"#bdET .sekme.bl{margin-bottom:8px;}" +
"#bdET .sekme button{flex:1;padding:7px 0;border:none;border-radius:8px;" +
 "font:800 12px/1 'Baloo 2',system-ui,sans-serif;color:#cfe0f7;" +
 "background:rgba(255,255,255,.10);transition:.09s;}" +
"#bdET .sekme.bl button{font-size:10.5px;letter-spacing:.3px;}" +
"#bdET .sekme button:active{transform:scale(.96);filter:brightness(.93);}" +
"#bdET .sekme button.secili{background:#ffd84d;color:#1b3a6b;" +
 "text-shadow:none;}" +
"#bdET .sekme.bl button.secili{background:#3d7ccc;color:#ffffff;}" +
...
"#bdET input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;" +
 "width:14px;height:14px;margin-top:-5.5px;border-radius:50%;background:#ffd84d;border:none;}" +
"#bdET input[type=range]::-moz-range-track{height:3px;border-radius:2px;" +
 "background:rgba(255,255,255,.22);}" +
"#bdET input[type=range]::-moz-range-thumb{width:14px;height:14px;border:none;" +
 "border-radius:50%;background:#ffd84d;}" +

/* SAYI KUTUSU — sürgünün aralığı bağlamaz, buraya elle yazılır. */
"#bdET input.dg{flex:0 0 46px;width:46px;padding:4px 0;border:none;" +
 "border-radius:6px;background:rgba(4,12,28,.45);color:#ffd84d;" +
 "font:700 11px/1 'Baloo 2',system-ui,sans-serif;text-align:center;" +
 "font-variant-numeric:tabular-nums;-moz-appearance:textfield;}" +
"#bdET input.dg::-webkit-outer-spin-button," +
"#bdET input.dg::-webkit-inner-spin-button{-webkit-appearance:none;margin:0;}" +

"#bdET .dgm{display:flex;gap:6px;margin-top:9px;}" +
...
/* [anahtar, ad, sürgü en az, sürgü en çok] — sınır yalnız sürgüye
   aittir, sayı kutusu bağlı değildir. */
var BOLUMLER = [
  ["cerceve", "ÇERÇEVE", [
    ["kPunto",  "Yazı boyu",        5,   80],
    ["kGenis",  "En çok genişlik", 20,  400],
    ["kDolguY", "Yatay dolgu",      0,   60],
    ["kDolguD", "Dikey dolgu",      0,   40],
    ["kKose",   "Köşe",             0,   40],
    ["kDx",     "Yatay kayma",   -280,  280],
    ["kDy",     "Dikey kayma",   -280,  280]
  ]],
  ["gorsel", "SEVİYE GÖRSELİ", [
    ["kGEn",    "Genişlik",         4,  400],
    ["kGBoy",   "Yükseklik",        4,  400],
    ["kGX",     "Yatay boşluk",  -280,  280],
    ["kGY",     "Dikey kayma",   -280,  280]
  ]],
  ["dugum", "DÜĞÜM (%)", [
    ["dPunto",  "Yazı boyu",       10,  200],
    ["dYaziX",  "Yatay kayma",   -300,  300],
    ["dYaziY",  "Dikey kayma",   -300,  500],
    ["dGEn",    "Görsel en",       10,  600],
    ["dGBoy",   "Görsel boy",      10,  600],
    ["dGX",     "Görsel yatay",  -300,  300],
    ["dGY",     "Görsel dikey",  -300,  300]
  ]]
];
...
ic += '<div class="sekme sv">';
SEVIYELER.forEach(function(n){
  ic += '<button data-sv="' + n + '"' + (n === sv ? ' class="secili"' : '') + '>' + n + '</button>';
});
ic += '</div><div class="sekme bl">';
BOLUMLER.forEach(function(b){
  ic += '<button data-bl="' + b[0] + '"' + (b[0] === bolum ? ' class="secili"' : '') + '>' + b[1] + '</button>';
});
ic += '</div>';

BOLUMLER.forEach(function(b){
  ic += '<div class="grup' + (b[0] === bolum ? ' acik' : '') + '" data-grup="' + b[0] + '">';
  b[2].forEach(function(f){
    ic += '<div class="sat"><span class="ad">' + f[1] + '</span>' +
          '<input type="range" data-k="' + f[0] + '" min="' + f[2] + '" max="' + f[3] +
          '" step="1" value="' + deger(f[0]) + '">' +
          '<input type="number" class="dg" data-k="' + f[0] + '" value="' + deger(f[0]) + '"></div>';
  });
  ic += '</div>';
});
...
/* Sayı kutusuna sürgü aralığının DIŞINDA bir değer yazılabilir.
   O durumda sürgü uca dayanır ama gerçek değer korunur — sürgüye
   dokunulmadıkça kutudaki sayı geçerlidir. */
function tazele(kaynak){
  var girisler = kutu.querySelectorAll("input[data-k]");
  for (var i = 0; i < girisler.length; i++) {
    var el = girisler[i], k = el.dataset.k, v = deger(k);
    if (el === kaynak) continue;
    if (+el.value !== v) el.value = v;
  }
  var dgm = kutu.querySelectorAll(".sekme button");
  for (var j = 0; j < dgm.length; j++) {
    var b = dgm[j];
    if (b.dataset.sv) b.classList.toggle("secili", +b.dataset.sv === sv);
    else b.classList.toggle("secili", b.dataset.bl === bolum);
  }
  var gruplar = kutu.querySelectorAll(".grup");
  for (var g = 0; g < gruplar.length; g++) {
    gruplar[g].classList.toggle("acik", gruplar[g].dataset.grup === bolum);
  }
}
...
function yerlestir(){
  ...
  kutu.addEventListener("input", function(ev){
    ...
    if (!k) return;
    /* Kutu boşaltılırken (silip yeniden yazarken) 0'a düşürme. */
  ...
  kutu.addEventListener("pointerup", function(ev){
    ...
    /* Sekme geçişi — ekranda hiçbir şey kıpırdamaz, yalnız panelin
       sürdüğü sayı kümesi ya da görünen bölüm değişir. */
    if (t.dataset && t.dataset.sv) { sv = +t.dataset.sv; tazele(); cikti.style.display = "none"; return; }
    if (t.dataset && t.dataset.bl) { bolum = t.dataset.bl; tazele(); return; }
...
/* ═══════════════════════════════════════════════════════════════
   KALKAN KUBBESİ  —  haritada kalkanı açık kalenin koruma alanı

   TEK BLOK. Daha önce bu dosyada bu bloğun İKİ KOPYASI oluştu;
   sonraki kopyanın <style>'ı öncekini ezdiği için düzeltmeler
   çalışmıyor gibi göründü. Yeni sürüm eklerken eskisini SİL,
   sonuna ikinci bir kopya ekleme.
   ...
   baktığı için taban dairesi elipse döner, kubbe ÖNDE zeminin
   altına taşar, arkada taşmaz. Yol = üstte yarım çember yayı +
   altta yarım elips yayı.

   Halkalar da aynı hesaptan gelir. z yüksekliğindeki enlem
   çemberi için:
       yarıçap  = r * karekök(1 - z*z)
       basıklık = yarıçap * sin(aci)
       dikey    = -r * cos(aci) * z
   Halka tepede sıfır yarıçapla doğar, inerken küreyi takip ederek
   genişler, en altta TAM TABAN ÇEMBERİ olarak biter. Kenara
   kendiliğinden yapışır; ayrı bir kapsam ayarı gerekmez.

   ── NEDEN CSS ANİMASYONU YOK ──
   index.html ~2122'de erişilebilirlik kuralı var:
     @media (prefers-reduced-motion:reduce){
       *{animation-duration:.01ms !important;
         animation-iteration-count:1 !important} }
   Telefonda "animasyonları azalt" açıksa her CSS animasyonu
   görünmeden biter; halka doğar doğmaz sıfır yarıçapa dönüp
   KAYBOLUR. Belirtisi tam olarak şudur: kubbe görünür (animasyonu
   yok), halkalar görünmez. O kural delinmez — halkalar kendi
   requestAnimationFrame döngümüzde çizilir. Toz efektinde de aynı
   yol izlenmişti.
   ...
   ── calc İÇİNDE İŞARET ──
   calc(-50% + -2.1%) GEÇERSİZDİR ve tarayıcı bildirimin TAMAMINI
   atar; kubbe ortalanmaz, kaydırma sürgüleri hiçbir şey yapmaz,
   konsolda hata da çıkmaz. Negatif değer "- x" olarak yazılır.

   ── SVG KİMLİKLERİ ──
   Gradyan ve kırpma yolu url(#kimlik) ile çağrılır; kimlik belge
   genelinde benzersiz olmalı. İki kalede aynı kimlik olsaydı
   ikincisi birincinin tanımını kullanırdı. Sayaçla ayrılıyor.
...
(function kalkanKubbesi() {
  ...
  function svgYaz(no) {
    ...
    /* Faz halkanın dizideki sırasından gelir ve data-faz'da saklanır;
       kare döngüsü onu okur. */
    /* ELİPS DEĞİL PATH: halka artık dalgalı, yani yarıçapı açıya göre
       sinüsle oynayan kapalı bir eğri. <ellipse> bunu yapamaz. */
    var pay = (AY.aralik / 100) / AY.adet, halka = "";
    for (var j = 0; j < AY.adet; j++) {
      halka += '<path d="" fill="none" stroke="' +
               rgba(zc, AY.zDolu / 100) +
               '" stroke-width="' + (AY.kal / 10 * 2).toFixed(2) +
               '" stroke-linejoin="round"' +
               ' data-faz="' + (j * pay).toFixed(4) + '"/>';
    }

    return '<svg class="kk-svg" viewBox="-115 -115 230 230">' +
      '<defs><radialGradient id="' + gid + '" cx="50%" cy="50%" r="50%">' +
        /* BUZLU CAM: eskiden duraklar 0.32 → 1.00 idi, yani kenar
           göbeğin üç katı koyulukta patlıyordu ve küre içi boş bir
           halka gibi görünüyordu. Referansta madde HER YERDE aynı,
           kenarda yalnız hafif bir kalınlaşma var — ışığın daha çok
           cam katından geçtiği yer orası. Rampa buna göre düzleştirildi. */
        '<stop offset="0%"  stop-color="' + rgba(kc, d * 0.62) + '"/>' +
        '<stop offset="62%" stop-color="' + rgba(kc, d * 0.70) + '"/>' +
        '<stop offset="90%" stop-color="' + rgba(kc, d * 0.92) + '"/>' +
  ...
  /* ── HALKA KARE DÖNGÜSÜ ──
     Elemanlar dizide tutulur; her karede DOM sorgusu yapılmaz.
     Dizi yalnız kubbe eklenip çıkarıldığında tazelenir. Kubbe yoksa
     döngü kendini durdurur, boşa kare harcamaz. */
  var _halka = [], _donuyor = false;

  function halkaTazele() {
    _halka = [];
    try {
      var l = document.querySelectorAll("#battleMap .kk-halkalar > path");
      for (var i = 0; i < l.length; i++) _halka.push(l[i]);
    } catch (e) {}
  ...
  function kare() {
    ...
    /* Dalga fazı ayrıca yavaşça dönüyor: tepeler sabit dursaydı halka
       büyürken şekil donuk kalır, "dalga" değil "yıldız" gibi okunurdu. */
    var dFaz = t * Math.PI * 2 * 0.35;

    for (var i = 0; i < _halka.length; i++) {
      var e = _halka[i];
  ...
  /* Ayar paneli değer değiştirince kubbe SİLİNMEZ, içeriği yerinde
     yeniden yazılır. Silip tara()'nın yeniden çizmesini beklemek
     saniyeye varan boşluklar yaratıyordu; sürgüyü oynatırken
     görüntü sürekli kesiliyordu. */
  window.KALKAN_YENILE = function () {
    stilYaz();
    var l = document.querySelectorAll("#battleMap .map-node.castle-node .kk-kubbe");
    for (var i = 0; i < l.length; i++) l[i].innerHTML = svgYaz(++_no);
    halkaTazele();
  };
...
/* ═══════════════════════════════════════════════════════════════
   ...
   oturtmak içindir. Beğenilen değerler yukarıdaki AY nesnesine
   KALICI yazılır ve BU BLOK SİLİNİR (?isik=1 ile aynı mantık).
   ═══════════════════════════════════════════════════════════════ */
(function kalkanAyar() {
  "use strict";
  ...
  function kur() {
    ...
    /* Android'de readonly textarea kopyalama menüsünü vermez ve
       ...
       API'si denenir, olmazsa geçici textarea. İkisi de olmazsa
       metin ekranda zaten okunur duruyor. */
...
/* ══════════════════════════════════════════════════════════════
   ELMAS SİMGESİ — İKİ AYRI ÖLÇÜ + YER BAŞINA AYAR
   ------------------------------------------------------------
   SORUN: <img> akışın içinde durduğu için boyu büyüdükçe satırı,
   düğmeyi, HUD hapını da büyütüyordu.
   ...
   YER BAŞINA AYAR: elmasIkon("magaza") → <span class="elmas-kutu
   ey-magaza">. Her yerin kendi `.ey-*` kuralı --el-* değişkenlerini
   ezer. Kural yazılmamışsa aşağıdaki :root varsayılanı geçerlidir.

   TUZAK — calc: kaydırma `calc(-50% + var(--el-x))` ile yazılamaz.
   Değer eksi olunca `calc(-50% + -0.2em)` çıkar, bu GEÇERSİZ CSS'tir
   ve transform'un TAMAMI sessizce düşer. İki ayrı translate zincirli.

   TUZAK — kırpılma: taşan görsel, üst kutusunda overflow varsa
   kesilir (overflow-y yatayda da kırpar).
   ══════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════
   ...
   Üstte yer kutucukları, altta beş sürgü. Her yer ayrı kaydedilir.
   İş bitince KOPYALA çıktısını kalıcı CSS'e geçir, BU BLOĞU SİL.
   ══════════════════════════════════════════════════════════════ */
(function elmasAyarPaneli() {
"use strict";
if (!/[?&]elmasayar=1/.test(location.search)) return;

/* Yer listesi — elmasIkon(anahtar) ile birebir aynı olmalı. */
var YERLER = [
  { id: "hud",      ad: "HUD",      not: "Üst şerit elmas hapı" },
  { id: "magaza",   ad: "Mağaza",   not: "Ürün kartı fiyat düğmesi" },
  { id: "insaat",   ad: "İnşaat",   not: "BİTİR düğmeleri (iki yer)" },
  { id: "kahraman", ad: "Kahraman", not: "Liste kilit fiyatı" },
  { id: "gelistir", ad: "Geliştir", not: "Panel Satın Al düğmesi" },
...
var VARSAYILAN = { kutu: 1.15, boy: 1.15, x: 0, y: 0, hiza: -0.20 };

var A;
try { A = JSON.parse(localStorage.getItem(ANAHTAR)) || {}; } catch (e) { A = {}; }
YERLER.forEach(function (y) {
  if (!A[y.id]) A[y.id] = {};
  for (var k in VARSAYILAN) if (typeof A[y.id][k] !== "number") A[y.id][k] = VARSAYILAN[k];
});
...
/* Çanta ::before'un kutusu ve ölçeği ayrı yoldan gider; boy/kutu
   oranı scale'e çevrilir, böylece sürgüler her yerde aynı anlamda. */
function cantaKurali(v) {
  var olcek = v.kutu > 0 ? v.boy / v.kutu : 1;
  return "#panel-inventory .inv-summary .stat-card .num::before{" +
           "width:" + d2(v.kutu) + "em;height:" + d2(v.kutu) + "em;" +
           "vertical-align:" + d2(v.hiza) + "em;" +
           "transform:scale(" + d2(olcek) + ") translate(" + d2(v.x) + "em," + d2(v.y) + "em);}";
}

function yerKurali(id, v) {
  return ".elmas-kutu.ey-" + id + "{" +
           "--el-kutu:" + d2(v.kutu) + "em;" +
           "--el-boy:"  + d2(v.boy)  + "em;" +
           "--el-x:"    + d2(v.x)    + "em;" +
           "--el-y:"    + d2(v.y)    + "em;" +
           "--el-hiza:" + d2(v.hiza) + "em;}";
}

function ciktiMetni() {
  var out = [];
  YERLER.forEach(function (y) {
    out.push(y.id === "canta" ? cantaKurali(A[y.id]) : yerKurali(y.id, A[y.id]));
  });
  return out.join("\n");
}
...
function uygula() {
  stil.textContent = ciktiMetni();
  var c = document.getElementById("eaCikti");
  if (c) c.textContent = ciktiMetni();
}

var SURGULER = [
  { alan: "kutu", ad: "Kutu",    min: 0.60, max: 2.20, adim: 0.05, akis: true  },
  { alan: "boy",  ad: "Görsel",  min: 0.60, max: 4.00, adim: 0.05, akis: false },
  { alan: "x",    ad: "Sağ/sol", min: -1.20, max: 1.20, adim: 0.02, akis: false },
  { alan: "y",    ad: "Yukarı",  min: -1.20, max: 1.20, adim: 0.02, akis: false },
  { alan: "hiza", ad: "Hiza",    min: -0.90, max: 0.40, adim: 0.02, akis: true  }
];
...
function kur() {
  ...
  function ciz() {
    var v = A[secili];
    var yer = YERLER.filter(function (y) { return y.id === secili; })[0];

    var sekmeler = YERLER.map(function (y) {
      var s = (y.id === secili);
      return '<button class="ea-sek" data-yer="' + y.id + '" style="flex:0 0 auto;' +
             'padding:3px 9px;border:none;border-radius:12px;' +
             'font:700 11px/1 \'Baloo 2\',sans-serif;' +
             (s ? 'background:#2DC9FC;color:#08192b;' : 'background:#1d3f63;color:#bfe9ff;') +
             '">' + y.ad + '</button>';
    }).join("");

    var surgu = SURGULER.map(function (g) {
      return '<div style="display:flex;align-items:center;gap:6px;margin:3px 0;">' +
               '<span style="width:52px;flex:0 0 auto;' + (g.akis ? 'color:#ffd257;' : '') + '">' + g.ad + '</span>' +
               '<input type="range" data-alan="' + g.alan + '" min="' + g.min + '" max="' + g.max +
                 '" step="' + g.adim + '" value="' + v[g.alan] + '" style="flex:1;min-width:0;height:18px;">' +
               '<span data-deger="' + g.alan + '" style="width:42px;flex:0 0 auto;text-align:right;' +
                 'font-variant-numeric:tabular-nums;color:#7fe4ff;">' + d2(v[g.alan]) + '</span>' +
             '</div>';
    }).join("");

    var ornek = (secili === "canta")
      ? '<span style="font-size:22px;">Çanta örneği panelde değil — çantayı aç</span>'
      : '<span class="elmas-kutu ey-' + secili + '"><img class="elmas-gor" src="elmas.webp" alt="" ' +
    ...
    p.innerHTML =
      '<div id="eaBaslik" style="display:flex;align-items:center;gap:6px;">' +
        ...
        '<button id="eaSifirla" style="padding:3px 8px;border:none;border-radius:7px;' +
    ...
    p.querySelectorAll(".ea-sek").forEach(function (b) {
      b.addEventListener("click", function () { secili = b.dataset.yer; kaydet(); ciz(); });
    });

    p.querySelectorAll('input[type="range"]').forEach(function (r) {
      r.addEventListener("input", function () {
        var alan = r.dataset.alan;
        A[secili][alan] = parseFloat(r.value);
        var dd = p.querySelector('[data-deger="' + alan + '"]');
        if (dd) dd.textContent = d2(A[secili][alan]);
        kaydet();
        uygula();
      });
    });
    ...
    document.getElementById("eaKopya").addEventListener("click", function () {
      ...
      function eskiYol() {
        try {
          ...
          var ok = document.execCommand("copy");
          ta.remove();
          b.textContent = ok ? "ALINDI" : "OLMADI";
        } catch (e2) { b.textContent = "OLMADI"; }
      }
      try {
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(metin)
...
(function hastaneOlcuVarsayilan() {
  ...
  st.textContent =
    "html body #panel-hospital{" +
      /* Üst */ "--hs-ust:71.5px;--hs-kap-fs:16.5px;--hs-alt-ic:14px;--hs-kaydir-ust:48.5px;--hs-yan:4.5px;--hs-kaydir-yan:4.5px;" +
      /* Kart */ "--hs-yuz:45px;--hs-kart-ic:6px;--hs-ikon-x:1.5px;" +
      /* Sürgü */ "--hs-ray:9px;--hs-ray-bas:12px;--hs-ray-bit:12.5px;--hs-adet-fs:15px;--hs-adet-en:78px;" +
      /* Topuz */ "--hs-top-en:15px;--hs-top-boy:23.5px;--hs-top-yuv:3px;--hs-top-bas:19.5px;--hs-top-bit:16px;--hs-top-cz-ara:4px;--hs-top-cz-en:1.5px;--hs-top-cz-boy:12.5px;" +
      /* Şerit */ "--hs-ikon:23px;--hs-yuva-ara:15.5px;--hs-ikon-sayi:4px;--hs-serit-fs:12px;--hs-serit-ic:6px;" +
      /* Düğme */ "--hs-btn-en:125.5px;--hs-btn-fs:13px;--hs-btn-ic:7px;--hs-btn-ara:30px;--hs-btn-ust:3.5px;" +
      /* Kuyruk */ "--hs-kuyruk-yuz:35.5px;}";
  document.head.appendChild(st);
})();
...
(function hastaneAyar() {
  ...
  var GRUP = [
    { ad: "Üst", ler: [
      { k: "--hs-ust",       ad: "Üst boşluk", v: 71.5,   min: 16, max: 96 },
      { k: "--hs-kap-fs",    ad: "Üst sayaç", v: 16.5, min: 9,  max: 22 },
      { k: "--hs-alt-ic",    ad: "Alt aralık", v: 14,    min: 0,  max: 26 },
      { k: "--hs-kaydir-ust", ad: "Liste üst", v: 48.5,   min: 0,  max: 70 },
      { k: "--hs-yan",       ad: "Kart yanı",  v: 4.5,   min: 0,  max: 30 },
      { k: "--hs-kaydir-yan", ad: "Liste yanı", v: 4.5,  min: 0,  max: 60 }
    ]},
    { ad: "Kart", ler: [
      { k: "--hs-yuz",       ad: "Görsel",     v: 45,   min: 30, max: 84 },
      { k: "--hs-kart-ic",   ad: "Satır iç",   v: 6,    min: 0,  max: 20 },
      { k: "--hs-ikon-x",    ad: "Görsel yatay", v: 1.5,  min: -60, max: 60 }
    ]},
    { ad: "Sürgü", ler: [
      { k: "--hs-ray",       ad: "Ray kalın",  v: 9,    min: 2,  max: 18 },
      { k: "--hs-ray-bas",   ad: "Ray başı",   v: 12,    min: 0,  max: 60 },
      { k: "--hs-ray-bit",   ad: "Ray sonu",   v: 12.5,    min: 0,  max: 60 },
      { k: "--hs-adet-fs",   ad: "Adet yazı",  v: 15, min: 9,  max: 22 },
      { k: "--hs-adet-en",   ad: "Adet kutu",  v: 78,   min: 34, max: 130 }
    ]},
    { ad: "Topuz", ler: [
      { k: "--hs-top-en",     ad: "Genişlik",   v: 15,  min: 5,   max: 34 },
      { k: "--hs-top-boy",    ad: "Yükseklik",  v: 23.5,  min: 10,  max: 44 },
      { k: "--hs-top-yuv",    ad: "Köşe",       v: 3,   min: 0,   max: 16 },
      { k: "--hs-top-bas",    ad: "Yol başı",   v: 19.5,   min: -20, max: 50 },
      { k: "--hs-top-bit",    ad: "Yol sonu",   v: 16,   min: -20, max: 50 },
      { k: "--hs-top-cz-ara", ad: "Çizgi ara",  v: 4,   min: 1,   max: 16 },
      { k: "--hs-top-cz-en",  ad: "Çizgi kalın", v: 1.5, min: 0.5, max: 5 },
      { k: "--hs-top-cz-boy", ad: "Çizgi boy",  v: 12.5,  min: 3,   max: 34 }
    ]},
    { ad: "Şerit", ler: [
      { k: "--hs-ikon",      ad: "Simge",      v: 23,   min: 14, max: 44 },
      { k: "--hs-yuva-ara",  ad: "Yuva arası", v: 15.5,   min: 0,  max: 30 },
      { k: "--hs-ikon-sayi", ad: "Simge-sayı", v: 4,    min: 0,  max: 24 },
      { k: "--hs-serit-fs",  ad: "Sayı yazı",  v: 12,   min: 9,  max: 24 },
      { k: "--hs-serit-ic",  ad: "Şerit iç",   v: 6,    min: 1,  max: 20 }
    ]},
    { ad: "Düğme", ler: [
      { k: "--hs-btn-en",    ad: "Genişlik",   v: 125.5,  min: 80, max: 220 },
      { k: "--hs-btn-fs",    ad: "Yazı",       v: 13,   min: 8,  max: 20 },
      { k: "--hs-btn-ic",    ad: "Yükseklik",  v: 7,    min: 2,  max: 20 },
      { k: "--hs-btn-ara",   ad: "Arası",      v: 30,   min: 0,  max: 30 },
      { k: "--hs-btn-ust",   ad: "Üst boşluk", v: 3.5,   min: 0,  max: 30 }
    ]},
    { ad: "Kuyruk", ler: [
      { k: "--hs-kuyruk-yuz", ad: "Görsel",    v: 35.5,   min: 22, max: 64 }
    ]}
  ];
...
/* ══════════════════════════════════════════════════════════════
   ...
   biyom sınırı: CFG.serpme.genislik + CFG.sinirYumusak.
...
var SURGU = [
  { ad: "siklik",  etiket: "Yığın sıklığı",   alt: 30, ust: 300, bolen: 1000, v: 101 },
  { ad: "ayrinti", etiket: "Kenar kıvrımı",   alt: 0,  ust: 70,  bolen: 100,  v: 14  },
  { ad: "esik1",   etiket: "Koyu alan (eşik 1)", alt: 10, ust: 80, bolen: 100, v: 30 },
  { ad: "esik2",   etiket: "Açık alan (eşik 2)", alt: 20, ust: 90, bolen: 100, v: 30 },
  { ad: "yum",     etiket: "Kenar yumuşaklığı", alt: 5, ust: 200, bolen: 1000, v: 70  },
  { ad: "kabarti", etiket: "Kabartı",         alt: 0,  ust: 150, bolen: 100,  v: 0  },
  { ad: "isikAci", etiket: "Işık yönü (°)",   alt: 0,  ust: 359, bolen: 1,    v: 109 },
  { ad: "kaydir",  etiket: "Kabartı eni",     alt: 2,  ust: 30,  bolen: 10,   v: 2   },
  { ad: "icTon",   etiket: "Fırça geçişi",    alt: 0,  ust: 60,  bolen: 100,  v: 58  },
  /* Biyom sınırı — CFG.boya'da DEĞİL, hedef alanıyla ayrılıyor */
  { ad: "serpme",  etiket: "Sınır pürüzü",    alt: 0,  ust: 80,  bolen: 1000, v: 58, hedef: "serpme" },
  /* Boyalı zeminde sınır artık PİKSEL cinsinden (CFG.sinirYumusak);
     eski gecisBandi yalnız ESKİ yolda geçerli olduğu için bu sürgü
     ona değil sinirYumusak'a bağlandı. */
  { ad: "gecis",   etiket: "Sınır yumuşaklığı (px)", alt: 1, ust: 80, bolen: 1, v: 4, hedef: "gecis" }
];
...
function uygula(tazele){
  ...
  SURGU.forEach(function (s) {
    ...
    else if (s.hedef === "gecis") C.sinirYumusak = d;
    else B[s.ad] = d;
  });
  B.acik = !!A.acik;
...
function kopyaMetni(){
  ...
  SURGU.forEach(function (s) {
    var d = A[s.ad] / s.bolen;
    var ad = s.hedef === "serpme" ? "serpme.genislik" : s.hedef === "gecis" ? "sinirYumusak" : s.ad;
...
/* ══════════════════════════════════════════════════════════════
   ZEMİN AYAR PANELİ 2  —  ?zeminayar=2
   ------------------------------------------------------------
   Adres satırına ?zeminayar=2 eklenmedikçe HİÇBİR ŞEY yapmaz.
   ?zeminayar=1 panelinden bağımsız; ikisi aynı anda açılmaz.

   NE: harita.js boyalı zeminin TÜM ayarları, sekmeli:
     GENEL  → yığın deseni, ton eşikleri, kabartı, biyom sınırı,
              çözünürlük (CFG.boya, serpme, sinirYumusak, zeminHD,
              zeminAdim)
     KAR / ÇİMEN / LAV → bölge başına ton kenar keskinliği,
              doygunluk, canlılık, parlaklık, kontrast ve 5 renk
              (CFG.boya.bolge + CFG.boya.palet)

   NEDEN varsayılanlar dosyadan OKUNUYOR: panel açılışta
   HARITA.CFG'nin kopyasını alır, SIFIRLA ona döner. Varsayılan
   iki yerde yazılı olmadığı için panel açılınca zemin zıplamaz.

   KOPYALA → "ZEMIN2 {...}" (JSON). YAPIŞTIR → aynı metni geri
   ...
   ══════════════════════════════════════════════════════════════ */
(function zeminAyarPaneli2(){
"use strict";

if (!/[?&]zeminayar=2(&|$)/.test(location.search)) return;

var ANAHTAR = "bdZemin2", KONUM = "bdZemin2Konum";

/* [yol, etiket, alt, üst, adım] — yol CFG içindeki nokta yolu */
var GENEL = [
  ["boya.siklik",   "Yığın sıklığı",          0.03, 0.30, 0.001],
  ["boya.ayrinti",  "Kenar kıvrımı",          0,    0.70, 0.01 ],
  ["boya.esik1",    "Koyu alan eşiği",        0.10, 0.80, 0.01 ],
  ["boya.esik2",    "Açık alan eşiği",        0.10, 0.90, 0.01 ],
  ["boya.yum",      "Ton yumuşaklığı",        0.005,0.20, 0.001],
  ["boya.icTon",    "Fırça geçişi",           0,    0.80, 0.01 ],
  ["boya.kabarti",  "Kabartı",                0,    1.50, 0.01 ],
  ["boya.isikAci",  "Işık yönü (°)",          0,    359,  1    ],
  ["boya.kaydir",   "Kabartı eni",            0.1,  3,    0.1  ],
  ["serpme.genislik","Sınır pürüzü",          0,    0.08, 0.001],
...
var BOLGE_AYAR = [
  ...
  ["doygunluk", "Doygunluk",            0,   2.5, 0.01],
  ["canlilik",  "Canlılık",            -1,   2,   0.01],
  ["parlaklik", "Parlaklık",           -0.5, 0.5, 0.01],
  ["kontrast",  "Kontrast",             0.5, 2,   0.01]
];
var TON = ["Gölge", "Koyu", "Orta", "Açık", "Parlak"];
...
function birlestir(h, k){ for (var a in k) { if (k[a] && typeof k[a] === "object" && !Array.isArray(k[a]) && h[a] && typeof h[a] === "object") birlestir(h[a], k[a]); else h[a] = k[a]; } return h; }
function ondalik(adim){ var s = String(adim); return s.indexOf(".") < 0 ? 0 : s.length - s.indexOf(".") - 1; }
function hex(c){ return "#" + c.map(function (x) { x = Math.round(x); return (x < 16 ? "0" : "") + x.toString(16); }).join(""); }
function rgb(h){ return [parseInt(h.substr(1, 2), 16), parseInt(h.substr(3, 2), 16), parseInt(h.substr(5, 2), 16)]; }

var VARSAYILAN = null, D = null, sekme = "genel", zam = null;

/* Durum: panelin sürdüğü alanların TAMAMI, CFG ile aynı şekil */
function durumAl(c){
...
function uygula(){
  var c = C(); if (!c) return;
  birlestir(c.boya, kopya(D.boya));
  c.serpme.genislik = D.serpme.genislik;
  ...
  var t = document.getElementById("z2Ac");
  if (t) t.textContent = D.boya.acik ? "YENİ" : "ESKİ";
}

function satir(yol, et, alt, ust, adim){
  var v = al(D, yol), od = ondalik(adim);
  return "<div class='z2S' data-yol='" + yol + "' data-adim='" + adim + "' data-od='" + od + "'>" +
    "<div class='z2E'><span>" + et + "</span>" +
    "<button class='z2B' data-y='-'>−</button><b>" + Number(v).toFixed(od) + "</b>" +
    "<button class='z2B' data-y='+'>+</button></div>" +
    "<input type='range' min='" + alt + "' max='" + ust + "' step='" + adim + "' value='" + v + "'></div>";
}

function govdeCiz(){
  var g = document.getElementById("z2Govde"), h = "";
  if (sekme === "genel") {
    GENEL.forEach(function (r) { h += satir(r[0], r[1], r[2], r[3], r[4]); });
  } else {
    BOLGE_AYAR.forEach(function (r) {
      h += satir("boya.bolge." + sekme + "." + r[0], r[1], r[2], r[3], r[4]);
    });
    h += "<div class='z2Ren'>";
    for (var i = 0; i < 5; i++) {
      h += "<label><input type='color' data-ton='" + i + "' value='" +
           hex(D.boya.palet[sekme][i]) + "'><i>" + TON[i] + "</i></label>";
    }
    h += "</div>";
  }
  g.innerHTML = h;
  document.querySelectorAll("#z2Sek button").forEach(function (b) {
    b.classList.toggle("on", b.getAttribute("data-s") === sekme);
  });
}
...
function yaz(s, v){
  ...
  koy(D, s.getAttribute("data-yol"), v);
  r.value = v;
  s.querySelector("b").textContent = v.toFixed(od);
  uygula();
}

function metin(){ return "ZEMIN2 " + JSON.stringify(D); }

function ciz(){
  var st = document.createElement("style");
  st.textContent =
    "#z2{position:fixed;z-index:99999;width:196px;font:11px/1.25 'Baloo 2',system-ui,sans-serif;" +
    ...
    "#z2Bas{display:flex;align-items:center;gap:4px;padding:5px 6px 4px 8px;cursor:move}" +
    "#z2Bas b{flex:1;font-size:11px;letter-spacing:.5px}" +
    "#z2 button{font:700 10px system-ui,sans-serif;color:#e8f4ff;background:rgba(255,255,255,.12);" +
     "border:none;border-radius:5px;padding:3px 6px}" +
    "#z2 button:active{filter:brightness(1.3)}" +
    "#z2Sek{display:flex;gap:3px;padding:0 6px 4px}" +
    "#z2Sek button{flex:1;padding:3px 0}" +
    "#z2Sek button.on{background:#2f7fd8}" +
    "#z2Govde{max-height:46vh;overflow-y:auto;padding:0 8px;touch-action:pan-y}" +
    ".z2S{margin:0 0 3px}" +
    ".z2E{display:flex;align-items:center;gap:3px}" +
    ".z2E span{flex:1;font-weight:700;font-size:10px;opacity:.9;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}" +
    ".z2E b{min-width:34px;text-align:center;font-variant-numeric:tabular-nums;font-size:10px}" +
    "#z2 .z2B{padding:0;width:20px;height:18px;font-size:13px;line-height:18px}" +
    ".z2S input{width:100%;height:14px;margin:0;touch-action:pan-x}" +
    ".z2Ren{display:flex;justify-content:space-between;margin:4px 0 6px}" +
    ".z2Ren label{display:flex;flex-direction:column;align-items:center;gap:1px}" +
    ".z2Ren input{width:30px;height:24px;border:none;padding:0;background:none}" +
    ".z2Ren i{font-style:normal;font-size:9px;opacity:.8}" +
    "#z2Alt{display:flex;gap:3px;padding:5px 6px 6px}" +
    "#z2Alt button{flex:1}" +
    "#z2Yap{display:none;padding:0 6px 6px}" +
    "#z2Yap textarea{width:100%;height:56px;box-sizing:border-box;font:9px monospace;" +
     "color:#dfeaff;background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.2);border-radius:5px}" +
    "#z2Not{font-size:9px;color:#9fc3ea;padding:0 8px 5px;min-height:0}";
  document.head.appendChild(st);

  var p = document.createElement("div");
  p.id = "z2";
  p.innerHTML =
    "<div id='z2Bas'><b>⠿ ZEMİN</b><button id='z2Ac'></button><button id='z2Kuc'>▾</button></div>" +
    "<div id='z2Ic'>" +
      "<div id='z2Sek'><button data-s='genel'>Genel</button><button data-s='kar'>Kar</button>" +
      "<button data-s='cimen'>Çimen</button><button data-s='lav'>Lav</button></div>" +
      "<div id='z2Govde'></div>" +
      "<div id='z2Alt'><button id='z2Kop'>KOPYALA</button><button id='z2YapA'>YAPIŞTIR</button>" +
      "<button id='z2Sif'>SIFIRLA</button></div>" +
      "<div id='z2Yap'><textarea id='z2Metin' placeholder='ZEMIN2 {...} metnini buraya yapıştır'></textarea>" +
      "<button id='z2Uyg' style='width:100%;margin-top:3px'>UYGULA</button></div>" +
      "<div id='z2Not'></div>" +
    "</div>";
  document.body.appendChild(p);

  /* Konum: kayıtlıysa oraya, yoksa sol üst. Ekran dışına taşarsa içeri al. */
  var k = null; try { k = JSON.parse(localStorage.getItem(KONUM) || "null"); } catch (e) {}
  function yerlestir(x, y){
    var r = p.getBoundingClientRect();
    /* Yatayda panelin TAMAMI ekranda kalır (başlıktaki ▾ düğmesi
       dışarı kaçmasın); dikeyde en az başlık görünür kalır. */
  ...
  function not(t){ var n = document.getElementById("z2Not"); n.textContent = t || ""; }

  document.getElementById("z2Sek").addEventListener("click", function (e) {
    var s = e.target.getAttribute && e.target.getAttribute("data-s");
    if (s) { sekme = s; govdeCiz(); }
  });

  var gov = document.getElementById("z2Govde");
  gov.addEventListener("input", function (e) {
    var t = e.target;
    if (t.type === "range") yaz(t.closest(".z2S"), parseFloat(t.value));
    else if (t.type === "color") {
      D.boya.palet[sekme][+t.getAttribute("data-ton")] = rgb(t.value);
      uygula();
    }
  });
  gov.addEventListener("click", function (e) {
    ...
    if (!y) return;
    var s = e.target.closest(".z2S");
    var adim = parseFloat(s.getAttribute("data-adim"));
    yaz(s, al(D, s.getAttribute("data-yol")) + (y === "+" ? adim : -adim));
  });

  document.getElementById("z2Ac").addEventListener("click", function () {
    D.boya.acik = !D.boya.acik; uygula();
  });

  var kucuk = false;
  document.getElementById("z2Kuc").addEventListener("click", function () {
    kucuk = !kucuk;
    document.getElementById("z2Ic").style.display = kucuk ? "none" : "block";
    this.textContent = kucuk ? "▴" : "▾";
    p.style.width = kucuk ? "auto" : "196px";
  });

  document.getElementById("z2Kop").addEventListener("click", function () {
    var t = metin(), ta = document.getElementById("z2Metin");
    ta.value = t;
    document.getElementById("z2Yap").style.display = "block";
    var ok = false;
  ...
  document.getElementById("z2YapA").addEventListener("click", function () {
    var y = document.getElementById("z2Yap");
    y.style.display = y.style.display === "block" ? "none" : "block";
    not("");
  });

  document.getElementById("z2Uyg").addEventListener("click", function () {
    var t = document.getElementById("z2Metin").value;
    var a = t.indexOf("{"), b = t.lastIndexOf("}");
    try {
      var yeni = JSON.parse(t.slice(a, b + 1));
      D = birlestir(kopya(VARSAYILAN), yeni);
      uygula(); govdeCiz(); not("Uygulandı.");
    } catch (e) { not("Metin okunamadı — ZEMIN2 {...} biçiminde olmalı."); }
  });

  document.getElementById("z2Sif").addEventListener("click", function () {
    D = kopya(VARSAYILAN); uygula(); govdeCiz(); not("Dosyadaki değerlere dönüldü.");
  });

  /* Sürükleme yalnız başlıktan; düğmelere basınca sürüklenmez */
  (function surukle(){
    var bas = document.getElementById("z2Bas"), aktif = false, bx, by, sx, sy;
...
/* ═══════════════════════════════════════════════════════════════
   BİRLİK EĞİTİM EKRANI — ALT KAMP SEKMELERİ · YAN DÜĞMELER ·
   NİTELİKLER  (referans düzeni, BÖLÜM 1)

   Serdar referans getirdi; üç parça:
     1) En altta üç kamp sekmesi, seçili olan açık zeminli ve yukarı
        çıkık. Aile değiştirmenin görünür tek yolu artık bu.
     2) Sağda alt alta iki düğme: ☰ Nitelikler · ⬆ Terfi.
     3) ☰ ile açılan Nitelikler ekranı — yedi satır, çubuklu,
        iki sütun. Ana ekrandaki dört stat kutusu buraya taşındı.

   Blok DOSYANIN SONUNDA: #panel-troops'un ölçü ve düzen kuralları
   yukarıda birkaç yere dağılmış durumda, sonra eklenen kazanıyor
   (Tuzak 38). Yeni kural yazarken buraya yaz, yukarıyı ezme.
   ═══════════════════════════════════════════════════════════════ */
(function birlikEkraniReferansDuzen(){
"use strict";
const st = document.createElement("style");
st.id = "temaBirlikReferans";
st.textContent = `
/* ── ALT KAMP SEKMELERİ ──
...
#panel-troops .uv-kamp-tab{
  ...
  font-family:'Baloo 2','Nunito',sans-serif !important;
  font-weight:800 !important; font-size:12px !important; line-height:1.1 !important;
  text-shadow:0 1px 2px rgba(0,20,45,.55) !important;
  box-shadow:0 2px 6px rgba(0,20,45,.3) !important;
...
/* İçerik sekmelerin altında kalmasın (62px + telefon çene payı) */
#panel-troops .unit-screen{
...
/* ── NİTELİKLER EKRANI ──
   ...
   Açıkken adet çubuğu ve Anında/Üret satırı gizlenir; kademe
...
#panel-troops .uvn-baslik{
  display:flex !important; align-items:center !important; gap:10px !important;
  margin:2px 2px 8px !important;
  font-family:'Baloo 2','Nunito',sans-serif !important;
  font-weight:900 !important; font-size:14px !important; color:#eaf7ff !important;
  text-shadow:0 1px 2px rgba(0,20,45,.55) !important;
}
#panel-troops .uvn-baslik::before,
#panel-troops .uvn-baslik::after{
  content:"" !important; flex:1 1 0 !important; height:1px !important;
  background:rgba(190,240,255,.35) !important;
}
#panel-troops .uvn-izgara{
  display:grid !important; grid-template-columns:1fr 1fr !important;
...
#panel-troops .uvn-ad{
  font-family:'Baloo 2','Nunito',sans-serif !important;
  font-weight:800 !important; font-size:12px !important; color:#dff4ff !important;
  white-space:nowrap !important; overflow:hidden !important; text-overflow:ellipsis !important;
  text-shadow:0 1px 2px rgba(0,20,45,.55) !important;
}
#panel-troops .uvn-dg{
  font-family:'Baloo 2','Nunito',sans-serif !important;
  font-weight:900 !important; font-size:13px !important; color:#fff !important;
  font-variant-numeric:tabular-nums !important;
  text-shadow:0 1px 2px rgba(0,20,45,.55) !important;
}
#panel-troops .uvn-ray{
  height:7px !important; margin-top:3px !important;
  border-radius:5px !important; overflow:hidden !important;
  background:rgba(4,32,60,.45) !important;
}
#panel-troops .uvn-dolu{
  display:block !important; height:100% !important; border-radius:5px !important;
  background:linear-gradient(180deg,#7ff08a,#33c65a) !important;
}
/* Oyunda karşılığı olmayan satır (Hız · Yük): sönük, çubuk boş */
#panel-troops .uvn-bos{ opacity:.55 !important; }
`;
document.head.appendChild(st);
})();
