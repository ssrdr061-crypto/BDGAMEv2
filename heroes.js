...
function openHeroDetail(skinId) {
  ...
  ov.innerHTML = `
    ...
    ${h.passive ? `<button id="hdPassive" style="position:absolute;top:58px;right:12px;z-index:10;width:36px;height:36px;padding:0;border-radius:50%;background:rgba(0,0,0,.55);border:1px solid rgba(255,255,255,.4);color:#fff;font-size:14px;overflow:hidden;display:flex;align-items:center;justify-content:center;">${h.passive.icon ? `<img src="${h.passive.icon}" style="display:block;width:100%;height:100%;object-fit:fill;">` : "◈"}</button>` : ""}
    ...
    <div id="hdName" style="position:absolute;top:14px;left:50%;transform:translateX(-50%);z-index:5;font-family:'Baloo 2','Nunito',sans-serif;font-size:26px;font-weight:800;letter-spacing:1.5px;color:#ffffff;-webkit-text-stroke:4px #6f6f78;paint-order:stroke fill;text-shadow:none;white-space:nowrap;">${h.name}</div>
    ...
    <div id="hdGuc" style="position:absolute;top:52px;left:50%;transform:translateX(-50%);z-index:5;font-family:'Baloo 2','Nunito',sans-serif;font-size:15px;font-weight:800;letter-spacing:.5px;color:#ffffff;-webkit-text-stroke:3px #7a7a83;paint-order:stroke fill;text-shadow:none;white-space:nowrap;font-variant-numeric:tabular-nums;"><span id="hdGucDeger">${
  ...
  /* Yıldız şeridi burada ÇİZİLMEZ. Tek kaynak gelistir.js'in
     panelidir; burada ikinci bir şerit vardı ve panel açılana kadar
     bir kare boyunca kahraman adının üstünde görünüyordu. */
  ...
  ov.querySelector("#uiShowVals").onclick = () => {
    const newPosY = "0.0%";   /* yıldız şeridi kaldırıldı */
